...
export default {
  ...
  mounted(){
    ...
    .then(res=>{
      console.log(res)
      this.timeArr = res.data.data.listDate
      if(this.title == '客单价'){
        this.initData = res.data.data.listPerTicketSale
      }else if(this.title == '客单数'){
        this.initData = res.data.data.listGuestSingular
      }else if(this.title == '产品价格'){
        this.initData = res.data.data.listProductPrice
      }else if(this.title == '询盘数'){
        this.initData = res.data.data.listInquiry
      }
