...
export default {
  ...
  data() {
    return {
      dataTime: [],
      Num: [],
      name:['UV','PV','人均PV',"IP","移动UV占比","移动UV","跳出率"],
      list:[]
    };
  },
  mounted() {
  ...
  methods: {
    ...
    init(list) {
        console.log(list.pvList,"123123jjjjasjdaskdjkl")
        this.list = list
      this.Num = []; //长度
      this.dataTime = [];
      this.Num.push([...list.uvList]);
      this.Num.push([...list.pvList]);
      this.Num.push([...list.gdpPvList]);
      this.Num.push([...list.ipList]);
      this.Num.push([...list.dUvList]);
      this.Num.push([...list.sUvList]);
      this.Num.push(list.jumpOutRateList)
