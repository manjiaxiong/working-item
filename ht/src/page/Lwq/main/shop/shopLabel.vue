          ...
          <tr class="juz"
            ...
            <td>
              <div class="lwqimgswtach"
                   :class="item.tagState==0?'true':'flase'"
                   @click="changeState(item.id,item.tagState)"></div>
            </td>
            <td class="btn">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      submitLabel(){
        if(this.zt == 0){//添加
          var val = $('.addMemu .name input').val()//输入的标签名
          var state = ''
      ...
      xiugai (index) {
        ...
        $('.modifyMemu .name input').val(this.arr[index].tagNames)
        if(this.arr[index].tagState == 0){
      ...
      changeState (id,tagState) {
        console.log(id,tagState)
        api.changeTag({
          id:id,
          tagState:tagState==0 ? 1 : 0
        })
        .then(res=>{
          console.log(res)
          this.shuaxin()
