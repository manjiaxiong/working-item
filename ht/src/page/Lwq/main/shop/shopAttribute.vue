          ...
          <tr class="juz"
            ...
            <td>
              <div class="lwqimgswtach"
                   :class="item.attributeState==0?'true':'flase'"
                   @click="changeState(item.id,item.attributeState)"></div>
            </td>
            <td class="btn">
...
<script>
  ...
  export default {
    ...
    methods: {
      labelDetail(id){
        this.$router.push({
            path : "/index/main1/attributeDetail",
      ...
      submitLabel(){
        if(this.zt == 0){//添加
          var val = $('.addMemu .name input').val()//输入的属性名
          var state = ''
      ...
      xiugai (index) {
        ...
        $('.modifyMemu .name input').val(this.arr[index].attributeNames)
        if(this.arr[index].attributeState == 0){
      ...
      changeState (id,attributeState) {
        console.log(id,attributeState)
        api.updateAttribute({
          id:id,
          attributeState:attributeState==0 ? 1 : 0
        })
        .then(res=>{
          console.log(res)
          this.shuaxin()
